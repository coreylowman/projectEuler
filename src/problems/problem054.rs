...
impl Hand {
    ...
    fn assign_type(&mut self) {
        if self.is_a_run() && self.cards_in_same_suit() {
            if self.cards[0].val == 10 {
                ...
                // straight flush
                self.hand_type = HandType::StraightFlush(self.cards[0].val,
                                                         self.cards[0].suit.clone());
                self.val = 8;
            }
            return;
        }
    ...
    fn cards_in_same_suit(&self) -> bool {
        self.cards[0].suit == self.cards[1].suit && self.cards[0].suit == self.cards[2].suit &&
        self.cards[0].suit == self.cards[3].suit && self.cards[0].suit == self.cards[4].suit
    }

    fn is_a_run(&self) -> bool {
        let v = self.cards[0].val;
        self.cards[1].val == v + 1 && self.cards[2].val == v + 2 && self.cards[3].val == v + 3 &&
    ...
    fn is_three_of_a_kind(&self) -> u8 {
        if self.cards[0].val == self.cards[1].val && self.cards[0].val == self.cards[2].val {
            return self.cards[0].val;
        } else if self.cards[1].val == self.cards[2].val && self.cards[1].val == self.cards[3].val {
            return self.cards[1].val;
        } else if self.cards[2].val == self.cards[3].val && self.cards[2].val == self.cards[4].val {
            return self.cards[2].val;
        }
        0
    }
    ...
    fn is_four_of_a_kind(&self) -> u8 {
        ...
        if self.cards[1].val == v && self.cards[2].val == v && self.cards[3].val == v {
            return v;
        } else if self.cards[2].val == q && self.cards[3].val == q && self.cards[4].val == q {
            return q;
        }
        0
    }
    ...
    fn is_full_house(&self) -> (u8, u8) {
        if self.cards[0].val == self.cards[1].val && self.cards[2].val == self.cards[3].val &&
           self.cards[2].val == self.cards[4].val {
            return (self.cards[0].val, self.cards[2].val);
        } else if self.cards[0].val == self.cards[1].val && self.cards[0].val == self.cards[2].val &&
